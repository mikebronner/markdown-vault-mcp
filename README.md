...
| `MARKDOWN_VAULT_MCP_DEFAULT_SEARCH_MODE` | `keyword` | Mode used when a `search` call omits `mode` (`keyword`, `semantic`, or `hybrid`). Set it to `hybrid` to fuse keyword and vector results by default; it degrades to `keyword` when the vault has no embeddings, while an explicit `semantic`/`hybrid` still errors instead. |
