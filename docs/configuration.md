...
|---|---|---|---|
| `MARKDOWN_VAULT_MCP_DEFAULT_SEARCH_MODE` | `keyword` | `keyword` \| `semantic` \| `hybrid` | Mode used when a `search` call omits `mode`. The default preserves historical behaviour; set `hybrid` to fuse keyword and vector results on every unqualified search. When the configured default needs embeddings that this vault does not have, it degrades to `keyword` instead of raising, so a vault without embeddings stays searchable. An **explicit** `mode="semantic"` or `mode="hybrid"` still raises `EmbeddingsNotConfiguredError`, so a caller who asked for semantic search is never handed keyword results under a semantic label. An unknown value is rejected at config load. |
